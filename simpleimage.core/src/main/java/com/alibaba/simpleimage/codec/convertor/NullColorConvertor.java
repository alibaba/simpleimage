 ...
 * @author wendell
 */
public class NullColorConvertor implements ColorConvertor {

    /*
     * (non-Javadoc)
     * @see com.alibaba.simpleimage.codec.util.MapColorConvertor#convert(int[], int)
     */
            ...
            for (int j = 0; j < DCTSIZE; j++) {
                ...
                if (index < bounds) {
                    for (int c = 0; c < numOfComponents; c++) {
                        output[index++] = (byte) (input[inputOffset++] & 0xFF);
                    }
                } else {
